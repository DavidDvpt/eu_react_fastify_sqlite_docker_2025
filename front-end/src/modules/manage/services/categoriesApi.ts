...
import { parseCategories } from "../categoryParser";

import type { Categories, CategoryApis } from "../categoryTypes";
...
async function getCategories(): Promise<Categories> {
  const response = await axiosCrud(axiosInstance()).get<CategoryApis>(
    CATEGORIES_ROUTE
  );
